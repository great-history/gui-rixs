...
from edrixs.rixs_utils import *
from edrixs.wannier_ham import *
...
res = get_atom_data('Ni', v_name='3d', v_noccu=noccu, edge='L23')
...
si = rescale(slat_i, ([1, 2], [0.65]*2))
si[0] = get_F0('d', si[1], si[2])    # F0_dd

# Slater integrals for intermediate Hamiltonian with core-hole
sn = rescale(slat_n, ([1, 2, 4, 5, 6], [0.65, 0.65, 0.95, 0.7, 0.7]))
sn[0] = get_F0('d', sn[1], sn[2])     # F0_dd
sn[3] = get_F0('dp', sn[5], sn[6])    # F0_dp
...
# Run ED
eval_i, eval_n, trans_op = ed_1v1c_py(
